                ...
                ${todayTask ? `
                    <div class="bg-white rounded-xl shadow-lg p-8 hover:shadow-xl transition-shadow group">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-3">
                                <!-- Current focus action buttons -->
                                <div class="flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button 
                                        onclick="app.deleteTask(${todayTask.id})"
                                        class="p-2 text-red-500 hover:text-red-700 hover:bg-red-100 rounded transition-all"
                                        title="Delete this task (not interested anymore)"
                                    >
                ...
                <div class="space-y-2 max-h-64 overflow-y-auto" id="tasksList">
                    ${app.projectTasks.filter(t => !t.is_completed && t.id !== todayTask?.id).length > 0 ? 
                        app.projectTasks.filter(t => !t.is_completed && t.id !== todayTask?.id).map((task, index) => `
                            <div class="flex items-center p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors task-item group" 
                                ...
                                <!-- ADHD-friendly action buttons -->
                                <div class="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button 
                                        onclick="event.stopPropagation(); app.promoteTask(${task.id})"
                                        class="p-1.5 text-blue-500 hover:text-blue-700 hover:bg-blue-100 rounded transition-all"
                                        title="Make this my current focus"
                                    >
                                    ...
                                    <button 
                                        onclick="event.stopPropagation(); app.deleteTask(${task.id})"
            ...
            <div class="bg-white rounded-xl shadow-sm p-8">
                <div class="max-w-2xl mx-auto">
                    ...
                    <div class="flex justify-center items-center space-x-4">
                        <!-- Delete button for ADHD flexibility -->
                        <button 
                            onclick="app.deleteTask(${todayTask.id})"
                            class="flex items-center space-x-2 text-red-500 hover:text-red-700 px-4 py-2 rounded-lg hover:bg-red-50 transition-all"
                            title="Not interested in this task anymore"
                        >
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                            </svg>
